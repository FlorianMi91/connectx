...
trainer = set_trainer(env,"random")
...
def chose_options():
    ...
    if form.validate_on_submit():
        print("validation")
        # Training agent in first position (player 1) against the default random agent.
        trainer.train = env.train([None, dico_options[form.adversaire.data]])
        obs = trainer.train.reset()
...
def game():
    ...
    if check_valid_move(board=obs["board"],move=int(move)):
        obs, reward, done, info = trainer.train.step(int(move))
    else:
